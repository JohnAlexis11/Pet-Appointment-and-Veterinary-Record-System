...
import { FPChecker } from "@/app/scripts/fpcheck/fp-checker";
    ...
    await Promise.all(
      urlsToBlock.map(
        async ({ domain, isSilent, type: threatType, isAggressiveMode, source, id }) => {
          ...
          const fpChecker = new FPChecker();
          try {
            // We don't check silent rules against Hubble
            if (!isSilent) {
              const shouldBlock = await fpChecker.shouldBlock(id);
              if (!shouldBlock) {
                // if the URL is good, don't block it
                return null;
              }
            }
          } catch (error) {
            console.error("Error checking hubble for FP prevention", error);
          }
