...
import { injectCSS, injectJsFile, injectJavascript, isMV3, isBrowserPage } from "@/utils/utils.js";
...
import type { EasyListItems } from "@/content-scripts/types/easyListElements";
import { ProcessedRules } from "../heuristics-db/types";
...
if (!(window === window.top) && document.location.ancestorOrigins && document.location.ancestorOrigins.length > 0) {
    // Find the parent URL of the iframe using location.ancestorOrigins
    pageUrl = document.location.ancestorOrigins[0] || pageUrl;
    useLogging && console.debug("Parent URL of iframe detected via ancestorOrigins:", pageUrl);
}


// Variable to hold the timeout ID
// This needs to be accessible across multiple calls to the observer callback
let debounceTimerId: ReturnType<typeof setTimeout> | undefined = undefined;

// The delay in milliseconds (2 seconds)
...
document.addEventListener("DOMContentLoaded", function() {    
    getTabInfo().then(async (tabInfo:any) => {
        ...
        if (!shouldBlockAds) {
            useLogging && console.debug("EasyList blocking is not enabled for this tab.");
            return;
        }
        ...
        await hideHeuristicElements(tabInfo);
        ...
        const observer = new MutationObserver((mutations: MutationRecord[]) => {
            // Check if any div elements were added
            let hasDivAdditions = false;
            outerLoop: for (const mutation of mutations) {
                for (const node of Array.from(mutation.addedNodes)) {
                    if (node && (node as any).tagName && (node as any).tagName.toLowerCase() === "div") {
                        hasDivAdditions = true; // only mark as true if a div was added
                        useLogging && console.debug("MutationObserver: Div element added, setting hasDivAdditions to true.");
                        indexScope.shouldCheckAds = true; // Set the flag to indicate ads should
                        break outerLoop; // should exit both loops immediately
                    }
                }
            }

            // Only proceed if div elements were added
            if (!hasDivAdditions) {
                return;
            }

            // 1. Clear any existing timer.
            // If the page changes again before the 2 seconds are up,
            // this cancels the previously scheduled execution of heuristics.
            clearTimeout(debounceTimerId);

            // 2. Set a new timer.
            // This schedules hideHeuristicElements() to run after `debounceDelay` milliseconds.
            // If another mutation occurs, the timer will be cleared by the next call
            // to this observer before it can execute.
            debounceTimerId = setTimeout(() => {
                useLogging && console.log(`MutationObserver: No mutations detected for ${debounceDelay / 1000} seconds. Running heuristics.`);

                hideHeuristicElements(tabInfo).then(() => {
                    useLogging && console.debug("MutationObserver: Heuristic elements removed after debounce.");
                }).catch((err) => {
                    useLogging && console.debug("MutationObserver: Error removing heuristic elements after debounce: ", err);
                });
            }, debounceDelay);
...
if (host === "www.linkedin.com") {
    ...
    try {
        ...
        if (!tab || !tab.id) {
            console.debug("LIA: No valid tab found.");
            return;
        }
        const shouldBlockAds = await isAdsProtectionActive(tab.id, pageUrl);
        if (shouldBlockAds === false) {
...
if (instartDomains.some((domain) => host.endsWith(domain))) {
    ...
    window.addEventListener("message", async (event) => {
        ...
        if (
            ...
            const shouldEnableInstart = await isAdsProtectionActive(tab.id!, originUrl.toString());
            if (shouldEnableInstart === false) {
                return;
            }

            console.debug("INS: handleNotWhitelisted", {params});
            handleNotWhitelisted(params.prop);
        }
    });
...
let selectorsCache: ProcessedRules | {} = {};

async function hideHeuristicElements(tabInfo: any) {
    if (Object.keys(selectorsCache).length === 0) { // If selectors are not loaded, we need to download them
        selectorsCache = await downloadHeuristicsDB();
    }

    await processHeuristicsRules(selectorsCache, tabInfo.url);
